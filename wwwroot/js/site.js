...
function tr() {
    ...
    if (!reg.test(document.getElementById("t").value)) {
        alert("账号应该是字母或数字且长度最大20");
        return;
    }

    if (!reg.test(document.getElementById("tt").value)) {
    ...
    // 将账号和密码传到后端
    var account = document.getElementById("t").value;
    var password = document.getElementById("tt").value;
    // 将账号和密码打包成JSON格式
    ...
    // 使用 sessionStorage 将值保存起来
    
    // 发送POST请求
    var xhr = new XMLHttpRequest();
    xhr.open('POST', "/Home/Index3", true);
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status >= 200 && xhr.status < 300) {
            ...
            // 请求成功处理逻辑
            alert("登录成功");
            window.location.href = "/Home/Index4";// 跳转到另一个页面
        } else if (xhr.status >= 400 && xhr.status < 500) {
            alert(xhr.response.message + "账号不存在或账号和密码不能为空");
...
function tt() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            alert(xhr.response.message + "账号已经存在");
        } else if (xhr.status >= 500) {
            alert('服务器内部错误：' + xhr.response.message);
        }
